...
def resultMakerNew(save_path):
    ...
    for dirpath, dir_names, file_names in os.walk(memory_info_dir):
        for dir_name in dir_names:
            ...
            for result_dirpath, result_dir_names, result_file_names in os.walk(memory_info_dir+os.path.sep+dir_name):
                for result_file_name in result_file_names:
                    if result_file_name.startswith('meminfo_before_test_'):
                        ...
                        worksheet.merge_range('C2:D2', 'Memory Info Before Test (After Reboot)', yellow_title_format)
                        worksheet.write('C3', 'Test Time', light_blue_format)
                        worksheet.write('C4', 'Total RAM (MB)', light_blue_format)
